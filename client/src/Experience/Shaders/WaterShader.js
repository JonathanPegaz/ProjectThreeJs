...
const oceanVertexShader = `
  ...
  void main() {
    ...
    // Create wave animation by offsetting the vertex position
    float amplitude = 0.1;
    float frequency = 1.5;
    float phase = uTime;
    vec3 waveOffset = vec3(
      amplitude * sin(vPosition.y * frequency + phase),
      amplitude * sin(vPosition.y * frequency + phase),
      amplitude * sin(vPosition.x * frequency + phase)
    );
    vPosition += waveOffset;
...
const oceanFragmentShader = `
  ...
  void main() {
    // Set base color to a blue-green color
    vec3 baseColor = vec3(0.1, 0.5, 0.4);

    // Add some shading to the color based on the vertex normal
...
// #define
// WATER2_COL
// vec3(0.0, 0.4180, 0.6758)
// #define
// FOAM_COL
// vec3(0.8125, 0.9609, 0.9648)
// #define
// FOG_COL
// vec3(0.6406, 0.9453, 0.9336)
// #define
// SKY_COL
// vec3(0.0, 0.8203, 1.0)
...
//     vec2
//     a = 0.025 * cdir.xz / cdir.y; // Parallax offset
//     float
//     h = sin(uv.x + iTime); // Height at UV
//     uv += a * h;
...
//     vec3
//     ret = mix(WATER_COL, WATER2_COL, waterlayer(uv + dist.xy));
...
//         {
//             vec2
//             offset = vec2(0.5) * vec2(x, y) - vec2(0.25);
//             #else
//             vec2
//             offset = vec2(0.0);
//             #endif
//             // Camera stuff
//             vec2
//             uv = (fragCoord.xy + offset) / iResolution.xy;
//             vec3
//             cpos = vec3(0.0, 4.0, 10.0); // Camera position
//             vec3
//             cdir = pixtoray(uv);
...
//             const vec3
//             ocean = vec3(0.0, 1.0, 0.0);
//             float
//             dist = -dot(cpos, ocean) / dot(cdir, ocean);
//             vec3
//             pos = cpos + dist * cdir;
